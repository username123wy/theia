...
class StatesReconnectHandler {
	...
	def handle(ReconnectAction action,ILanguageAwareDiagramServer server){
		...
		server.diagramLanguageServer.languageServerAccess.doRead(server.sourceUri, [ context |
			...
			if (sourceElement instanceof UseCase && targetElement instanceof Actor) {
				...
				val usecasetName = sourceElement.getContainerOfType(UCModel)?.actor?.head?.name ?: 'undefined'
				val transitionText = '''«'\n\t'»«usecaseName» => «(targetElement as UseCase).name»'''
				val oldRange = getOldRange(routable)
				val newRange = getNewRange(sourceElement as UseCase)
				if(oldRange!==null){
	...
	private def getOldRange(SModelElement routable) {
	 ...
	 private def getNewRange(UseCase sourceElement) {
